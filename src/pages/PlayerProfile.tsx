  ...
  return (
    <div
      className={`border rounded-xl overflow-hidden transition-shadow hover:shadow-md ${
        match.won
          ? 'border-l-4 border-l-emerald-400 border-t-slate-100 border-r-slate-100 border-b-slate-100 dark:border-t-slate-800 dark:border-r-slate-800 dark:border-b-slate-800'
          : 'border-l-4 border-l-rose-400 border-t-slate-100 border-r-slate-100 border-b-slate-100 dark:border-t-slate-800 dark:border-r-slate-800 dark:border-b-slate-800'
      }`}
    >
      ...
      <div className="px-3 md:px-4 py-2.5 md:py-3">
        ...
        <div className={`flex items-center gap-2 md:gap-3 py-1 md:py-1.5 ${match.won ? 'font-bold' : ''}`}>
          <span
            className={`w-5 h-5 md:w-6 md:h-6 rounded-md flex items-center justify-center text-[10px] md:text-xs font-bold text-white shrink-0 ${
              match.won ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'
            }`}
          >
            {match.won ? 'W' : 'L'}
          </span>
          <div className="flex-1 min-w-0 overflow-hidden">
            <p className={`text-xs md:text-sm truncate ${match.won ? 'text-emerald-700' : 'text-slate-600 dark:text-slate-300'}`}>
              ...
              {match.partner && (
                <span className="text-slate-400 dark:text-slate-500"> / <PlayerNameLink name={match.partner} nameMap={nameMap} currentUsabId={playerUsabId} className={match.won ? 'text-emerald-700 hover:text-violet-600' : 'text-slate-600 dark:text-slate-300 hover:text-violet-600'} /></span>
              )}
            </p>
          </div>
              ...
              scores.map(([a, b], i) => (
                <span
                  ...
                  className={`text-xs md:text-sm font-mono tabular-nums ${
                    a > b ? 'text-emerald-700 font-bold' : 'text-slate-600 dark:text-slate-300 font-normal'
                  }`}
                >
                  {a}
                </span>
        ...
        <div className={`flex items-center gap-2 md:gap-3 py-1 md:py-1.5 ${!match.won ? 'font-bold' : ''}`}>
          <span
            className={`w-5 h-5 md:w-6 md:h-6 rounded-md flex items-center justify-center text-[10px] md:text-xs font-bold text-white shrink-0 ${
              !match.won ? 'bg-rose-500' : 'bg-slate-300 dark:bg-slate-600'
            }`}
          >
            {!match.won ? 'W' : 'L'}
          </span>
            ...
            {scores.length > 0 ? (
              scores.map(([a, b], i) => (
                <span
                  key={i}
                  className={`text-xs md:text-sm font-mono tabular-nums ${
                    b > a ? 'text-rose-700 font-bold' : 'text-slate-600 dark:text-slate-300 font-normal'
                  }`}
                >
                  {b}
                </span>
            ...
            {(() => {
              ...
              return (
                <div>
                  ...
                  >
                    <h5 className="text-[10px] md:text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                      Recent Results{statsTab !== 'total' ? ` — ${STATS_TABS.find((t) => t.key === statsTab)?.label}` : ''}
                    </h5>
                    <span className="text-[10px] md:text-xs text-slate-400 dark:text-slate-500">({filtered.length})</span>
                    <ChevronDown className={`w-3.5 h-3.5 text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-transform ${recentResultsExpanded ? 'rotate-180' : ''}`} />
                  </button>
