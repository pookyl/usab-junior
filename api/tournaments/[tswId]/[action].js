...
// ── Disk-based tournament cache (pre-scraped JSON under data/tournament-cache/) ─
...
async function serveDiskCache(tswId, action, queryParams) {
...
async function synthesizePlayerDetail(tswId, q) {
  ...
  try {
    ...
    for (const f of matchFiles) {
      ...
      for (const m of dayData.matches || []) {
        if ((m.team1Ids || []).includes(playerId) || (m.team2Ids || []).includes(playerId))
          allMatches.push(m);
      }
    }
  ...
  try {
    ...
    for (const f of eventFiles) {
      ...
      for (const entry of evData.entries || []) {
        if (!(entry.players || []).some(p => p.playerId === playerId)) continue;
        const partners = (entry.players || []).filter(p => p.playerId !== playerId).map(p => p.name);
        const label = partners.length > 0 ? `${evData.eventName} with ${partners.join(' / ')}` : evData.eventName;
  ...
  return JSON.stringify({
    tswId, playerId, playerName: player.name, memberId, club: player.club || '',
    events, winLoss: total > 0 ? { wins, losses, total, winPct: Math.round((wins / total) * 100) } : null,
    matches: allMatches, hasUpcomingMatches,
  });
}

async function synthesizePlayerSchedule(tswId, q) {
  const rawIds = q.playerIds || '';
  ...
  const players = playerIds.filter(id => playerMap.has(id)).map(id => ({ playerId: id, playerName: playerMap.get(id).name }));
  ...
  for (const eventName of upcomingEventNames) {
    ...
    const prefixed = drawsList.find(d => d.name.startsWith(eventName) && !/consolation/i.test(d.name) && !/play-?off/i.test(d.name));
  ...
  for (const [, drawObj] of eventToDrawObj) {
    ...
    if (!t.includes('elimination') || bracketCache.has(drawObj.drawId)) continue;
    try {
  ...
  function parseTime(timeStr) {
    ...
    if (m) {
      ...
      return { date, time: m[4].trim(), dateLabel: isNaN(d.getTime()) ? '' : d.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }) };
    }
  ...
  for (const m of allUpcoming) {
    ...
    const inTeam1 = (m.team1Ids || []).includes(pid);
    if (!inTeam1 && !(m.team2Ids || []).includes(pid)) continue;
    ...
    const partnerNames = [], partnerPlayerIds = [];
    for (let i = 0; i < playerTeamNames.length; i++) {
      if ((playerTeamIds[i] || null) !== pid) { partnerNames.push(playerTeamNames[i]); partnerPlayerIds.push(playerTeamIds[i] ?? null); }
    }
    let status = 'upcoming';
    if ((m.status || '').toLowerCase().includes('now')) status = 'in-progress';
    const drawObj = eventToDrawObj.get(m.event);
    const drawType = drawObj ? ((drawObj.type || '').toLowerCase().includes('elimination') ? 'elimination' : (drawObj.type || '').toLowerCase().includes('round') ? 'round-robin' : 'unknown') : 'unknown';
    if (!matchesByDate.has(dateKey)) matchesByDate.set(dateKey, { dateLabel: parsed.dateLabel || '', matches: [] });
    matchesByDate.get(dateKey).matches.push({
      playerId: pid, event: m.event || '', round: m.round || '', time: parsed.time || m.time || '', court: m.court || '',
      drawType, status, opponent: { names: opponentNames, playerIds: opponentIds },
      partner: partnerNames.length > 0 ? { names: partnerNames, playerIds: partnerPlayerIds } : null,
      result: null, nextMatches: [], consolation: null, consolationMatches: [],
    });
  }

  const days = [...matchesByDate.entries()].filter(([d]) => d !== 'unknown').sort(([a], [b]) => a.localeCompare(b))
    .map(([date, data]) => ({ date, dateLabel: data.dateLabel, matches: data.matches }));
  const unknownDay = matchesByDate.get('unknown');
  if (unknownDay?.matches.length) days.push({ date: '', dateLabel: '', matches: unknownDay.matches });
    ...
    if (mainSection) {
      ...
      if (deepestWinLevel !== Infinity) {
        ...
        if (currentLevel >= 1) {
          ...
          let consPath = findConsolationPath(bracket, playerId, currentLevel, consMainNum, consolationType)
...
export default async function handler(req, res) {
  ...
  if (!refresh) {
    const queryParams = {
      d: req.query?.d || urlObj.searchParams.get('d') || '',
      drawId: req.query?.drawId || urlObj.searchParams.get('drawId') || '',
      eventId: req.query?.eventId || urlObj.searchParams.get('eventId') || '',
      playerId: req.query?.playerId || urlObj.searchParams.get('playerId') || '',
      playerIds: req.query?.playerIds || urlObj.searchParams.get('playerIds') || '',
    };
